...
import { useUpdateCompetitionCommentMutation } from '@api/competition-api';


interface ICompetitionCommentEditModalProps {
    visible: boolean,
...
export const CompetitionCommentEditModal: React.FC<ICompetitionCommentEditModalProps> = (props) => {
...
const ModalContent: React.FC<IModalContent> = (props) => {
    ...
    const [updateCompetitionComment, { isLoading, error }] = useUpdateCompetitionCommentMutation();


    const onUpdateComment = async () => {
        try {
            if (newComment.trim() !== "") {
                await updateCompetitionComment({
                    commentId: commentId,
                    data: {
                        text: newComment
                    }
